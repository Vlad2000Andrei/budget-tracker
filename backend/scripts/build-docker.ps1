# PowerShell Script to Build and Push Backend Docker Image (Multi-Architecture)
# Supports running on Raspberry Pi (ARM64) and standard systems (AMD64).

# Automatically set working directory to the parent directory of this script (backend root)
...
Write-Host "Starting multi-architecture build and push for 'budget-tracker-backend'..." -ForegroundColor Cyan
Write-Host "Platforms: linux/amd64, linux/arm64" -ForegroundColor Yellow
Write-Host "Tags: $DockerHubAccount/budget-tracker-backend:$VersionTag, $DockerHubAccount/budget-tracker-backend:latest" -ForegroundColor Yellow

& docker buildx build --platform linux/amd64,linux/arm64 `
  -t "$DockerHubAccount/budget-tracker-backend:$VersionTag" `
  -t "$DockerHubAccount/budget-tracker-backend:latest" `
  --push .
...
Write-Host "Images: $DockerHubAccount/budget-tracker-backend:$VersionTag and $DockerHubAccount/budget-tracker-backend:latest" -ForegroundColor Green
