#!/bin/bash
# Bash Script to Build and Push Backend Docker Image (Multi-Architecture)
...
echo "Starting multi-architecture build and push for 'budget-tracker-backend'..."
echo "Platforms: linux/amd64, linux/arm64"
echo "Tags: $DOCKER_HUB_ACCOUNT/budget-tracker-backend:$VERSION_TAG, $DOCKER_HUB_ACCOUNT/budget-tracker-backend:latest"

docker buildx build --platform linux/amd64,linux/arm64 \
  -t "$DOCKER_HUB_ACCOUNT/budget-tracker-backend:$VERSION_TAG" \
  -t "$DOCKER_HUB_ACCOUNT/budget-tracker-backend:latest" \
  --push .

echo -e "\nSuccessfully built and pushed multi-architecture images to Docker Hub!"
echo "Images: $DOCKER_HUB_ACCOUNT/budget-tracker-backend:$VERSION_TAG and $DOCKER_HUB_ACCOUNT/budget-tracker-backend:latest"
